...
import (
	...
	"github.com/hayride-dev/bindings/go/internal/gen/exports/hayride/mcp/prompts"
	"go.bytecodealliance.org/cm"
)

// errorResource represents the error resource
type errorResource struct {
	Code prompts.ErrorCode
	Data string
}

// createError creates a new error resource and stores it in the resource table.
func createError(code prompts.ErrorCode, data string) prompts.Error {
	err := errorResource{
		Code: code,
		Data: data,
	}
	...
	return prompts.ErrorResourceNew(key)
}

func errorCode(self cm.Rep) prompts.ErrorCode {
	err, ok := resourceTable.errors[self]
	if !ok {
		return prompts.ErrorCodeUnknown
	}
