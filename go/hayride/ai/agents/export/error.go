...
import (
	...
	"github.com/hayride-dev/bindings/go/internal/gen/exports/hayride/ai/agents"
	"go.bytecodealliance.org/cm"
)

// errorResource represents the error resource
type errorResource struct {
	Code agents.ErrorCode
	Data string
}

// createError creates a new error resource and stores it in the resource table.
func createError(code agents.ErrorCode, data string) agents.Error {
	err := errorResource{
		Code: code,
		Data: data,
	}
	...
	return agents.ErrorResourceNew(key)
}

func errorCode(self cm.Rep) agents.ErrorCode {
	err, ok := resourceTable.errors[self]
	if !ok {
		return agents.ErrorCodeUnknown
	}
