...
// Package model represents the imported interface "hayride:ai/model@0.0.50".
package model
...
// GraphExecutionContextStream represents the imported type alias "hayride:ai/model@0.0.50#graph-execution-context-stream".
//
...
// Error represents the imported resource "hayride:ai/model@0.0.50#error".
//
//	resource error
...
// Format represents the imported resource "hayride:ai/model@0.0.50#format".
//
//	resource format
...
//
//go:nosplit
func (self Format) Model() (result string) {
	self0 := cm.Reinterpret[uint32](self)
	wasmimport_FormatModel((uint32)(self0), &result)
	return
}

// Model represents the imported resource "hayride:ai/model@0.0.50#model".
//
//	resource model
...
//
//go:nosplit
func NewModel(format Format, graph GraphExecutionContextStream) (result Model) {
	format0 := cm.Reinterpret[uint32](format)
	graph0 := cm.Reinterpret[uint32](graph)
	result0 := wasmimport_NewModel((uint32)(format0), (uint32)(graph0))
...
//
//go:nosplit
func (self Model) Compute(messages cm.List[Message]) (result cm.Result[MessageShape, Message, Error]) {
	self0 := cm.Reinterpret[uint32](self)
	messages0, messages1 := cm.LowerList(messages)
	wasmimport_ModelCompute((uint32)(self0), (*Message)(messages0), (uint32)(messages1), &result)
	return
}
