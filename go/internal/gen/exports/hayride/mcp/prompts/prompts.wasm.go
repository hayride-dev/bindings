// Code generated by wit-bindgen-go. DO NOT EDIT.

package prompts

import (
	"go.bytecodealliance.org/cm"
)

// This file contains wasmimport and wasmexport declarations for "hayride:mcp@0.0.62".

//go:wasmimport [export]hayride:mcp/prompts@0.0.62 [resource-new]error
//go:noescape
func wasmimport_ErrorResourceNew(rep0 uint32) (result0 uint32)

//go:wasmimport [export]hayride:mcp/prompts@0.0.62 [resource-rep]error
//go:noescape
func wasmimport_ErrorResourceRep(self0 uint32) (result0 uint32)

//go:wasmimport [export]hayride:mcp/prompts@0.0.62 [resource-drop]error
//go:noescape
func wasmimport_ErrorResourceDrop(self0 uint32)

//go:wasmexport hayride:mcp/prompts@0.0.62#[dtor]error
//export hayride:mcp/prompts@0.0.62#[dtor]error
func wasmexport_ErrorDestructor(self0 uint32) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	Exports.Error.Destructor(self)
	return
}

//go:wasmexport hayride:mcp/prompts@0.0.62#[method]error.code
//export hayride:mcp/prompts@0.0.62#[method]error.code
func wasmexport_ErrorCode(self0 uint32) (result0 uint32) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	result := Exports.Error.Code(self)
	result0 = (uint32)(result)
	return
}

//go:wasmexport hayride:mcp/prompts@0.0.62#[method]error.data
//export hayride:mcp/prompts@0.0.62#[method]error.data
func wasmexport_ErrorData(self0 uint32) (result *string) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	result_ := Exports.Error.Data(self)
	result = &result_
	return
}

//go:wasmimport [export]hayride:mcp/prompts@0.0.62 [resource-new]prompts
//go:noescape
func wasmimport_PromptsResourceNew(rep0 uint32) (result0 uint32)

//go:wasmimport [export]hayride:mcp/prompts@0.0.62 [resource-rep]prompts
//go:noescape
func wasmimport_PromptsResourceRep(self0 uint32) (result0 uint32)

//go:wasmimport [export]hayride:mcp/prompts@0.0.62 [resource-drop]prompts
//go:noescape
func wasmimport_PromptsResourceDrop(self0 uint32)

//go:wasmexport hayride:mcp/prompts@0.0.62#[dtor]prompts
//export hayride:mcp/prompts@0.0.62#[dtor]prompts
func wasmexport_PromptsDestructor(self0 uint32) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	Exports.Prompts.Destructor(self)
	return
}

//go:wasmexport hayride:mcp/prompts@0.0.62#[constructor]prompts
//export hayride:mcp/prompts@0.0.62#[constructor]prompts
func wasmexport_Constructor() (result0 uint32) {
	result := Exports.Prompts.Constructor()
	result0 = cm.Reinterpret[uint32](result)
	return
}

//go:wasmexport hayride:mcp/prompts@0.0.62#[method]prompts.get-prompt
//export hayride:mcp/prompts@0.0.62#[method]prompts.get-prompt
func wasmexport_PromptsGetPrompt(self0 uint32, params0 *uint8, params1 uint32, params2 *[2]string, params3 uint32) (result *cm.Result[GetPromptResultShape, GetPromptResult, Error]) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	params := lift_GetPromptParams((*uint8)(params0), (uint32)(params1), (*[2]string)(params2), (uint32)(params3))
	result_ := Exports.Prompts.GetPrompt(self, params)
	result = &result_
	return
}

//go:wasmexport hayride:mcp/prompts@0.0.62#[method]prompts.list-prompts
//export hayride:mcp/prompts@0.0.62#[method]prompts.list-prompts
func wasmexport_PromptsListPrompts(self0 uint32, cursor0 *uint8, cursor1 uint32) (result *cm.Result[ListPromptsResultShape, ListPromptsResult, Error]) {
	self := cm.Reinterpret[cm.Rep]((uint32)(self0))
	cursor := cm.LiftString[string]((*uint8)(cursor0), (uint32)(cursor1))
	result_ := Exports.Prompts.ListPrompts(self, cursor)
	result = &result_
	return
}
