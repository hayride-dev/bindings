// Code generated by wit-bindgen-go. DO NOT EDIT.

package types

// This file contains wasmimport and wasmexport declarations for "hayride:core@0.0.15".

//go:wasmimport hayride:core/types@0.0.15 [resource-drop]error
//go:noescape
func wasmimport_ErrorResourceDrop(self0 uint32)

//go:wasmimport hayride:core/types@0.0.15 [method]error.code
//go:noescape
func wasmimport_ErrorCode(self0 uint32) (result0 uint32)

//go:wasmimport hayride:core/types@0.0.15 [method]error.data
//go:noescape
func wasmimport_ErrorData(self0 uint32, result *string)
